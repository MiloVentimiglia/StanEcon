...
An important note on prior distributions: if a prior distribution puts no weight on a particular value of the parameter, the posterior distribution cannot put weight on that value either. We can use this property by choosing prior distributions that restrict estimates of $\beta$ to economically meaningful values. For instance, if we are estimating a very simple cost function $\mbox{costs} = \alpha + \beta \mbox{quantity sold} + \epsilon$, we may want to exclude the possibility of zero or negative fixed costs (that is, we think that $\alpha > 0$). In such a case, we could give $\alpha$ a prior distribution that is only defined for positive values, such as a truncated normal distribution, the lognormal distribution, or a gamma distribution. 
...
## Modeling the world: a generative approach


Economists build models of human economic behavior. Unlike modelers in other domains (like machine vision or voice recognition), economists tend not to build models of deterministic systems. The human behavior they wish to model is a moving target, with optimization, strategic interaction, the impact of expectations and true randomness all complicating the modeling task. Compared with the job of building a model to classify photographs, economic modeling is ambitious; after all, a photograph of a dog is always a photograph of a dog. 

At the same time, economists are not solely (or perhaps _not particularly_) interested in building models with strong predictive abilities. More commonly, economists will try to build models to evaluate the probable _policy impact_ of a proposed change. Central banks will want to know how inflation and unemployment responds to a change in the policy rate. Education departments will be interested in how much a training program is likely to affect their students. Corporations will be interested in how a price change will affect their market share. Regulators are interested in how two companies merging will affect consumer welfare. And so on. These are not predictive problems, in that 

What do I want to say? 

- Economic models as generative models
- Thinking about a model as a random number generator
- Strong philosophical foundations of generative modeling
- 
...
# How much additional income does one earn because of their level of education? 
...
### Fuzzy cut-off
## Panel data: a Bayesian approach
...
# How do consumers respond to an increase in the price of breakfast cereal?
...
# The Lucas critique and structural macroeconomic modeling
